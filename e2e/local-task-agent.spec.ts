...
test('creates a local workspace conversation, chats natively, and resumes without task bindings', async () => {
  ...
  try {
    ...
    const taskBounds = await page!.getByRole('main', { name: 'Task workspace' }).boundingBox()
    const chatBounds = await dialog.boundingBox()
    expect(taskBounds).not.toBeNull()
    expect(chatBounds).not.toBeNull()
    expect(chatBounds!.x).toBeGreaterThanOrEqual(taskBounds!.x + taskBounds!.width)
    expect(await dialog.evaluate((element) => element.parentElement?.className)).toBe('workbench-body')
