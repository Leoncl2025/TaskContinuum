...
test('links and streams original AHP chats with current receipts and explicit link recovery', async () => {
  ...
  try {
    ...
    expect(security.keys).toEqual(['bindCreation', 'cancel', 'create', 'createLocal', 'creationStatus', 'creationWorkers', 'creations', 'list', 'localCreationHosts', 'localCreationStatus', 'localCreations', 'models', 'onView', 'releaseTerminal', 'resolveDelivery', 'send', 'terminal', 'unwatch', 'watch'])
    ...
    const options = panel.getByRole('button', { name: 'Model options', exact: true })
    await expect(options).toHaveText('Medium · 272K')
    await expect(panel.getByRole('dialog', { name: 'Model options' })).toHaveCount(0)
    const compactHeight = await panel.locator('.composer').evaluate((element) => element.getBoundingClientRect().height)
    expect(compactHeight).toBeLessThanOrEqual(130)
    expect(await panel.locator('.composer-toolbar').evaluate((element) => element.getBoundingClientRect().height)).toBeLessThanOrEqual(40)
    await options.click()
    await expect(panel.getByRole('dialog', { name: 'Model options' })).toBeInViewport()
    expect(await panel.locator('.composer').evaluate((element) => element.getBoundingClientRect().height)).toBe(compactHeight)
    ...
    await expect(options).toHaveText('Max · 872K')
    await expect(panel.getByRole('dialog', { name: 'Model options' })).toHaveCount(0)
    ...
    await expect(options).toHaveText('Max · 872K')
    expect(await panel.locator('.composer').evaluate((element) => element.getBoundingClientRect().height)).toBeLessThanOrEqual(160)
    await page!.screenshot({ path: resolve('artifacts/agent-host-compact-composer.png') })
    await options.click()
    await expect(panel.getByRole('dialog', { name: 'Model options' })).toBeInViewport()
    ...
    await expect(restored.getByRole('button', { name: 'Model options', exact: true })).toHaveText('Max · 872K')
    await restored.getByRole('button', { name: 'Model options', exact: true }).click()
