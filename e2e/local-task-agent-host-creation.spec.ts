...
async function installLocalCreationMock(app: ElectronApplication, ledgerFile: string, workspaceId: string): Promise<void> {
  await app.evaluate(({ ipcMain, dialog, session }, { ledgerFile, workspaceId }) => {
    ...
    const handlers: Record<string, (...args: unknown[]) => unknown> = {
      'agent-host:list': () => { record('agent-host:list', []); return { sessions: [], warnings: [] } },
      ...
      'agent-host:create': (value) => {
        ...
        if (state.operations.length) throw new Error('A second native creation was attempted.')
        ...
        const operation: AgentHostCreation = {
          ...
          session: { ...target, title: 'Created local task chat', provider: 'copilotcli', canSend: true, updatedAt: '' },
        }
        state.snapshot.document.bindings[request.taskId] = { provider: 'agent-host', ...target }
...
test('creates a task-local chat through the real preload without remote creation or the workspace assistant', async () => {
  ...
  try {
    ...
    await writeFile(ledgerFile, JSON.stringify({
      worker, snapshot: { document: { schemaVersion: 2, bindings: {} }, revision: null, localOwner: owner }, operations: [], calls: [],
    ...
    expect(operation.session).toBeDefined()
    const target = { sessionId: operation.session!.sessionId, chatId: operation.session!.chatId, owner }
    await expect(panel.getByText(target.sessionId, { exact: true })).toBeVisible()
    ...
    expect(await calls('agent-host:create')).toEqual([{ channel: 'agent-host:create', args: [{ operationId: operation.operationId, taskId: 'T-0001', workerId: owner.clientId, workspaceId: worker.workspaces[0].id, hostId: 'exact-local-host', expectedRevision: revision }] }])
