...
export function registerWorkspaceBridge(requireWindow: (event: IpcMainInvokeEvent) => BrowserWindow, allowDirectory: (root: string) => void, remoteOwner?: (root: string, target: VSCodeChatTarget) => Promise<SessionOwner | undefined>, verifyAgentHost?: (root: string, target: AgentHostTarget) => Promise<AgentHostTarget>, onOpen?: (root: string) => Promise<void>) {
  ...
  async function authorize(state: WorkspaceState): Promise<WorkspaceState> {
    if (state.current) { allowDirectory(state.current.root); await onOpen?.(state.current.root) }
    return state
  }
  ...
  handle('migrate-session-links', async (_window, request) => { await authorize(await store.getState()); return store.migrateSessionLinks(request) })
