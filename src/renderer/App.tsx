...
function Workbench({ suppliedAdapter, workspaces }: { suppliedAdapter?: ChatAdapter; workspaces: ReturnType<typeof useWorkspaces> }) {
  ...
  useEffect(() => {
    ...
    if (!binding || binding.vscodeWorkspaceStorageId || current === binding.id || !copilot.bridge || !live || copilot.status.state !== 'ready') return
    let cancelled = false
