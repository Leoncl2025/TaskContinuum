...
export function RemoteDeviceConnections() {
  ...
  return <div className="remote-device-controls remote-connections">
    ...
    {devices?.map((device) => {
      const label = machineLabel({ clientId: device.ownerClientId, machineName: device.machineName }, aliases)
      return <div className="remote-vscode-row" key={device.id}>
        <div className="remote-vscode-row-title"><Icon name="device-desktop" /><strong title={device.machineName}>{label}</strong><span className="muted">{device.state}</span></div>
        {device.error && <p className="copilot-error">{device.error}</p>}
        <div className="remote-vscode-actions">
          <IconButton icon={device.enabled ? 'debug-disconnect' : 'plug'} label={`${device.enabled ? 'Disconnect device' : 'Connect device'} ${label}`} disabled={!!busy && busy !== device.id} onClick={() => {
          ...
          <IconButton icon="refresh" label={`Reconnect device ${label}`} disabled={!!busy} onClick={() => { void run(device.id, () => api.connect(device.id)) }} />
          <IconButton icon="trash" label={`Forget device ${label}`} disabled={!!busy} onClick={() => { void run(device.id, () => api.forget(device.id)) }} />
        </div>
      </div>
    })}
  </div>
