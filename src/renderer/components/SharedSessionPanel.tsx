...
interface SharedMessage { id: string; actor: SharedEvent['actor']; text: string; role: 'user' | 'assistant'; state?: string }
...
function timeline(events: SharedEvent[]): SharedMessage[] {
  ...
  for (const event of events) {
    ...
    if (event.type === 'message') messages.push({ id: `user-${event.commandId}`, actor: event.actor, text: event.text ?? '', role: 'user' })
...
export function SharedSessionPanel({ task, root, onClose, onStatus }: { task?: TaskRecord; root: string; onClose(): void; onStatus?(status: SharedPanelStatus): void }) {
  ...
  const submissions = useRef(new Map<string, { id: string; text: string }>())
  ...
  async function send(): Promise<void> {
    if (!bridge || !view || !canSend || !draft.trim()) return
    const currentDraft = draft
    const text = currentDraft.trim()
    const prior = submissions.current.get(view.session.id)
    const command = prior?.text === text ? prior : { id: crypto.randomUUID(), text }
    submissions.current.set(view.session.id, command)
    const sent = await shared.run(async () => { await bridge.send(view.session.id, command.id, command.text); return true })
    if (sent) {
  ...
  return <aside className="chat-panel shared-panel" aria-label="Shared session chat">
    ...
    <form className="composer-area" onSubmit={(event) => { event.preventDefault(); void send() }}><div className="composer"><textarea aria-label="Message shared Agent" rows={3} maxLength={4000} value={draft} onChange={(event) => setDrafts((current) => ({ ...current, [draftKey]: event.target.value }))} placeholder="Message shared Agent" onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send() } }} /><div className="composer-toolbar"><span><Icon name="server" />{view ? view.session.owner.machineName : 'No execution owner'}</span><button type="submit" className="send-button" aria-label="Send shared message" disabled={!canSend || !draft.trim()}><Icon name="arrow-up" /></button></div></div></form>
