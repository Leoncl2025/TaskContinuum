...
export function TaskViewer({ task, onChat, workspaceName = 'Task Continuum' }: { task: TaskRecord; onChat(): void; workspaceName?: string }) {
  const [tab, setTab] = useState<typeof tabs[number]>('Overview')
  const progress = taskProgress(task)
  const checked = task.checklist.filter((item) => item.done).length

  function checklist(limit?: number) {
    return <ul className="acceptance-list">{task.checklist.slice(0, limit).map((item) => <li key={item.id}><label><input type="checkbox" checked={item.done} disabled /><span className={item.done ? 'criterion-complete' : ''}>{item.title}</span><small>{item.id}</small></label></li>)}</ul>
  }
  ...
  return <div className="task-viewer" aria-label="Task viewer">
    ...
    <article id="task-document" className="task-document" role="tabpanel" aria-labelledby={`tab-${tab}`} tabIndex={0}>
      <div className="task-eyebrow"><span>{task.kind}</span><span>{task.id}</span><span className="sample-label">Read-only task</span></div>
      <h1>{task.title}</h1>
      <p className="task-summary">{task.summary}</p>
      <div className="metadata-row">
        <label className={`status-select status-${task.status}`}><span className="status-dot" /><select aria-label="Task status" value={task.status} disabled>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      {tab === 'Requirements' && <WorkspaceDocument source={task.documents?.requirements ?? null} />}
      {tab === 'Plan' && <WorkspaceDocument source={task.documents?.plan ?? null} />}
      {tab === 'Checklist' && <section className="document-section"><h2>Acceptance checklist</h2><p>{checked} of {task.checklist.length} criteria checked.</p>{checklist()}{!task.checklist.length && <p className="muted">No acceptance criteria recorded.</p>}</section>}
    </article>
  </div>
}