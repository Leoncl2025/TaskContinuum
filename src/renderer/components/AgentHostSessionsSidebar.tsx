    ...
    <li role="treeitem" aria-label={`${taskId} ${title}`} aria-expanded={expanded} aria-level={1}
      ...
      {expanded && <ul role="group" className="task-tree-group">
        {bindings.map((binding) => {
          ...
          const label = discovered?.title || (binding.title !== 'Agent Host' ? binding.title : binding.id)
...
export function AgentHostSessionsSidebar({ taskId, taskTitle, taskReady = false, bindings = {}, activeKey, localOwnerId, busy = false, initialView = 'current', onSelect, onDetach, onLink, onCreated, onDevices, onClose, onTasks }: Props) {
  ...
  useEffect(() => {
    ...
    void (bridge?.list() ?? Promise.reject(new Error('The Agent Host desktop API is unavailable.'))).then((result) => {
      if (active) { setSessions(result.sessions); setWarnings(result.warnings); setCatalogueError(undefined) }
  ...
  return <aside className="sidebar session-sidebar" aria-label="Agent Host sessions">
    ...
    <div className="sidebar-scroll session-sidebar-content" id="session-management-content" role="tabpanel" aria-label={`${view} sessions`}>
      ...
      {view === 'current' && <>
        {taskId && <SessionTree taskId={taskId} title={taskTitle ?? taskId} bindings={linked} activeKey={activeKey} sessions={sessions} disabled={!taskReady || busy || operating} onSelect={onSelect} onUnlink={onDetach ? (binding) => { setDetach(binding); setError(undefined) } : undefined} />}
