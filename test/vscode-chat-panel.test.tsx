...
describe('original VS Code conversation panel', () => {
  it('renders assistant Markdown while retaining literal user text and updated history', async () => {
    ...
    const snapshot: VSCodeChatView = { session: { id: 'original', title: 'Original', source: 'vscode', updatedAt: '' }, messages: [
      { id: 'user', role: 'user', text: '**Keep my source**', status: 'complete' },
      { id: 'answer', role: 'assistant', text: source, status: 'complete', nativeRequestId: 'same-request' },
    ] }
    ...
    await screen.findByRole('heading', { name: 'Results', level: 2 })
    ...
    expect(view.container.querySelector('.message-user .message-text')?.textContent).toBe('**Keep my source**')
    vi.mocked(bridge.read).mockResolvedValue({ ...snapshot, messages: [{ ...snapshot.messages[1], text: '### Updated\n\n```ts\nconst partial = 1', status: 'streaming' }] })
    act(() => changed())
    await screen.findByRole('heading', { name: 'Updated', level: 3 })
    expect(screen.queryByRole('heading', { name: 'Results' })).not.toBeInTheDocument()
    expect(screen.getByLabelText('Code block')).toHaveTextContent('const partial = 1')
    expect(screen.getByText('Responding in VS Code')).toBeInTheDocument()
    expect(view.container.querySelector('[data-request-id="same-request"]')).toBeInTheDocument()
    expect(bridge.open).not.toHaveBeenCalled()
  })

  it('never turns message HTML, links, or images into executable or network content', async () => {
    const { bridge } = fixture()
    vi.mocked(bridge.read).mockResolvedValue({ session: { id: 'original', title: 'Original', source: 'vscode', updatedAt: '' }, messages: [
      { id: 'answer', role: 'assistant', status: 'complete', text: '**Safe text**\n\n[Docs](https://example.com) [Run](command:workbench.action.closeWindow) [Script](javascript:alert%281%29) [File](file:///C:/secret)\n\n![Remote picture](https://example.com/track.png)\n\n<script>alert(1)</script>\n\n<iframe src="https://example.com"></iframe>\n\n<img src="https://example.com/pixel" onerror="alert(1)">\n\n```html\n<script>not executable</script>\n```' },
    ] })
    const view = render(<VSCodeChatPanel task={demoTasks[1]} identity={identity} onDetach={vi.fn()} onClose={vi.fn()} />)
    ...
    expect(view.container.querySelector('.message-markdown')?.querySelector('a, img, script, iframe, [href], [src], [onerror]')).toBeNull()
    expect(screen.getByLabelText('Code block')).toHaveTextContent('<script>not executable</script>')
    expect(bridge.open).not.toHaveBeenCalled()
  })

  it('copies only the code text and handles unavailable clipboard access', async () => {
    const { bridge, changed } = fixture()
    const user = userEvent.setup()
    const write = vi.spyOn(navigator.clipboard, 'writeText').mockResolvedValue()
    const snapshot: VSCodeChatView = { session: { id: 'original', title: 'Original', source: 'vscode', updatedAt: '' }, messages: [
      { id: 'answer', role: 'assistant', status: 'complete', text: '```ts\nconst answer = 42;\n```' },
    ] }
    vi.mocked(bridge.read).mockResolvedValue(snapshot)
    ...
    vi.mocked(bridge.read).mockResolvedValue({ ...snapshot, messages: [{ ...snapshot.messages[0], text: '```ts\nconst updated = 43;\n```' }] })
    act(() => changed())
