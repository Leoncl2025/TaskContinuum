...
function target(session = 'one', owner = b): SessionLink {
  return { provider: 'agent-host', owner: { clientId: owner.actor.deviceId, machineName: `machine-${owner.actor.deviceId.at(-1)}` }, sessionId: `copilotcli:/${session}`, chatId: `ahp-chat:/${session}` }
}
...
describe('canonical signed immutable remote records', () => {
  ...
  it.each([
    ...
    { name: 'ownerless Agent Host', target: { provider: 'agent-host', sessionId: 'copilotcli:/original', chatId: 'ahp-chat:/original' } },
