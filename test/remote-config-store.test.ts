...
describe('effective repository binding backend and durable immutable store', () => {
  ...
  it('persists trimmed Unicode aliases in the outbox across restart without altering original bindings', async () => {
    const f = await fixture()
    await expect(f.store.setMachineAlias(b.actor.deviceId, 'Office', null)).rejects.toThrow('before naming machines')
    const initial = await f.store.initialize()
    const linked = await f.store.writeBinding('T-0001', [binding(), binding('two', a)], initial.revision)
    const alias = '\u529e\u516c\u5ba4 workstation'
    const saved = await f.store.setMachineAlias(b.actor.deviceId, `  ${alias}  `, linked.revision)
    expect(saved.resolution.machineAliases).toEqual({ [b.actor.deviceId]: alias })
    expect(saved.document).toEqual(linked.document)
    const operation = saved.records.find((record) => record.kind === 'alias')!
    expect(operation.payload).toEqual({ action: 'set', deviceId: b.actor.deviceId, alias })
    expect(operation.actor).toEqual(a.actor)
    expect(await readRecords(f.outboxRoot)).toContainEqual(operation)
    f.changed.mockClear()
    expect(await f.store.setMachineAlias(b.actor.deviceId, alias, saved.revision)).toEqual(saved)
    expect(f.changed).not.toHaveBeenCalled()
    await f.store.close()
    const reopened = new RemoteConfigStore(f.options)
    stores.push(reopened)
    expect((await reopened.read()).resolution.machineAliases).toEqual({ [b.actor.deviceId]: alias })
    expect(await reopened.getPendingRecords()).toContainEqual(operation)
    const other = await fixture()
    expect((await other.store.initialize()).resolution.machineAliases).toEqual({})
  })

  it('rejects stale alias edits and writes a causal tombstone when clearing an alias', async () => {
    const f = await fixture()
    const initial = await f.store.initialize()
    expect(await f.store.setMachineAlias(b.actor.deviceId, '', initial.revision)).toEqual(initial)
    expect(await f.store.getPendingRecords()).toEqual([])
    const saved = await f.store.setMachineAlias(b.actor.deviceId, 'Office', initial.revision)
    await expect(f.store.setMachineAlias(b.actor.deviceId, 'Stale edit', initial.revision)).rejects.toThrow('changed on disk')
    await expect(f.store.setMachineAlias(b.actor.deviceId, 'x'.repeat(81), saved.revision)).rejects.toThrow('at most 80')
    await expect(f.store.setMachineAlias(b.actor.deviceId, 'Line\nbreak', saved.revision)).rejects.toThrow('single-line')
    expect((await f.store.read()).resolution.machineAliases).toEqual({ [b.actor.deviceId]: 'Office' })
    const cleared = await f.store.setMachineAlias(b.actor.deviceId, '  ', saved.revision)
    expect(cleared.resolution.machineAliases).toEqual({})
    const deletion = cleared.records.find((record) => record.kind === 'alias' && record.payload.action === 'delete')!
    expect(deletion.parents).toEqual([saved.records[0].operationId])
    expect(cleared.resolution.entities[`alias:${b.actor.deviceId}`].state).toBe('deleted')
    expect(await f.store.setMachineAlias(b.actor.deviceId, null, cleared.revision)).toEqual(cleared)
    expect(await f.store.getPendingRecords()).toHaveLength(2)
  })

  it.each([null, 'Resolved workstation'])('resolves alias conflicts using every observed head (%s)', async (alias) => {
    const f = await fixture()
    await f.store.initialize()
    const first = await operation({ kind: 'alias', payload: { action: 'set', deviceId: b.actor.deviceId, alias: 'Office' } })
    const second = await operation({ kind: 'alias', payload: { action: 'set', deviceId: b.actor.deviceId, alias: 'Laptop' } }, b)
    for (const record of [first, second]) await appendRecord(f.recordsRoot, record)
    const before = await f.store.read()
    expect(before.resolution.machineAliases).toEqual({})
    const saved = await f.store.setMachineAlias(b.actor.deviceId, alias, before.revision)
    expect((await f.store.getPendingRecords())[0].parents).toEqual([first.operationId, second.operationId].sort())
    expect(saved.resolution.machineAliases).toEqual(alias === null ? {} : { [b.actor.deviceId]: alias })
