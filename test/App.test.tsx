...
async function renderWorkspace() {
  ...
  await screen.findByRole('tab', { name: 'UI based on Electron' })
  return view
}
afterEach(() => {
...
describe('workbench', () => {
  ...
  it('places chat before task details and keeps task tabs accessible when details are hidden', async () => {
    await renderWorkspace()
    const main = screen.getByRole('main', { name: 'Task workspace' })
    const chat = within(main).getByRole('complementary', { name: 'Task chat' })
    const details = within(main).getByRole('complementary', { name: 'Task details' })
    expect(chat.compareDocumentPosition(details) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    ...
    expect(screen.getByRole('tab', { name: 'UI based on Electron' })).toBeVisible()
  ...
  it('resizes both panel edges with the keyboard and preserves hidden widths', () => {
    ...
    const details = screen.getByRole('separator', { name: 'Resize task details' })
    ...
    expect(details).toHaveAttribute('aria-valuenow', '430')
    expect(details).toHaveAttribute('aria-orientation', 'vertical')
    expect(JSON.parse(localStorage.getItem('taskcontinuum:layout:v2')!)).toMatchObject({ sidebarWidth: 308, detailsWidth: 430 })
    fireEvent.keyDown(window, { key: 'b', ctrlKey: true })
    ...
    expect(details).toHaveAttribute('aria-valuenow', details.getAttribute('aria-valuemax')!)
  ...
  it('restores widths, fits a smaller window without overwriting preferences, and resets sizing', async () => {
    ...
    expect(screen.getByRole('separator', { name: 'Resize task details' })).toHaveAttribute('aria-valuenow', '500')
    ...
    expect(JSON.parse(localStorage.getItem('taskcontinuum:layout:v2')!)).toMatchObject({ sidebarWidth: 400, detailsWidth: 500 })
    vi.stubGlobal('innerWidth', 1440)
    fireEvent.resize(window)
    expect(screen.getByRole('separator', { name: 'Resize task details' })).toHaveAttribute('aria-valuenow', '500')
    view.unmount()
    ...
    fireEvent.keyDown(screen.getByRole('separator', { name: 'Resize task details' }), { key: 'Enter' })
    expect(screen.getByRole('separator', { name: 'Resize Explorer' })).toHaveAttribute('aria-valuenow', '258')
    expect(screen.getByRole('separator', { name: 'Resize task details' })).toHaveAttribute('aria-valuenow', '380')
    ...
    expect(JSON.parse(localStorage.getItem('taskcontinuum:layout:v2')!)).toMatchObject({ sidebarWidth: 258, detailsWidth: 380, theme: 'light' })
  })
  ...
  it('persists display preferences and reports only native integration capabilities', async () => {
    ...
    expect(localStorage.getItem('taskcontinuum:layout:v2')).toContain('light')
  })
  ...
  it('opens chat after compact task selection and preserves document tabs across pane switches', async () => {
    ...
    const user = userEvent.setup()
    await renderWorkspace()
    const preferences = localStorage.getItem('taskcontinuum:layout:v2')
    expect(screen.queryByRole('separator')).not.toBeInTheDocument()
    expect(screen.getByRole('navigation', { name: 'Workspace panes' })).toBeInTheDocument()
    expect(screen.getByRole('complementary', { name: 'Task chat' })).toBeVisible()
    expect(screen.queryByRole('complementary', { name: 'Task details' })).not.toBeInTheDocument()
    expect(screen.queryByRole('complementary', { name: 'Task explorer' })).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Tasks' }))
    expect(screen.queryByRole('main')).not.toBeInTheDocument()
    await user.click(explorer().getByRole('button', { name: 'T-0003 Backend service' }))
    expect(screen.getByRole('complementary', { name: 'Task chat' })).toHaveTextContent('No Agent Host chat is linked to T-0003')
    expect(screen.queryByRole('complementary', { name: 'Task explorer' })).not.toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: 'A path forward' })).toBeVisible()
    fireEvent.keyDown(window, { key: 'Escape' })
    expect(screen.getByRole('complementary', { name: 'Task chat' })).toBeVisible()
    expect(localStorage.getItem('taskcontinuum:layout:v2')).toBe(preferences)
  })

  it('keeps first-run guidance reachable in the compact details pane', () => {
    vi.spyOn(window, 'matchMedia').mockImplementation((query) => ({ matches: true, media: query, onchange: null, addEventListener: vi.fn(), removeEventListener: vi.fn(), addListener: vi.fn(), removeListener: vi.fn(), dispatchEvent: () => true }))
    render(<App />)
    expect(screen.getByRole('heading', { name: 'Create your task repository' })).toBeVisible()
    expect(screen.getByRole('button', { name: 'Toggle task details' })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.queryByRole('complementary', { name: 'Task chat' })).not.toBeInTheDocument()
  })
